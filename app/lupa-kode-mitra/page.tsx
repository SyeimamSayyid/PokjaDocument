...
export default function LupaKodeMitraPage() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email.trim() || !noWa.trim()) { setError('Email dan No HP wajib diisi.'); return; }
    setError(''); setLoading(true);
    try {
      const res = await fetch('/api/auth/lupa-kode-mitra', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ email: email.trim(), noWa: noWa.trim() }),
      });
      const data = await res.json();

      if (res.ok) {
        setPesan(data.message || 'Kode akses Anda telah dikirim ulang ke email Anda.');
        setDone(true);
      } else {
    ...
    <div className="auth-page">
      <style>{`
        ...
        .auth-glow-1 {
          ...
          opacity: 0.16; animation: glowDrift1 16s ease-in-out infinite;
        }
        .auth-glow-2 {
          width: 500px; height: 500px; bottom: -170px; left: -130px;
          background: radial-gradient(circle, ${CREAM} 0%, transparent 70%);
          opacity: 0.14; animation: glowDrift2 19s ease-in-out infinite;
        }
        ...
        .auth-title { font-size: 16px; font-weight: 800; letter-spacing: 0.04em; color: ${INK}; text-align: center; }
        .auth-subtitle { font-size: 11.5px; color: #64748b; line-height: 1.6; max-width: 290px; text-align: center; }
      ...
      <div className="auth-card">
        {done ? (
          <>
            ...
            <a href="/login-mitra" className="auth-footer">
              <FiArrowLeft size={13} /> Kembali ke Akses Dokumen
            </a>
          </>
        ) : (
          <>
            <div className="auth-brand-icon"><FaBuilding size={22} /></div>
            <div className="auth-title">Lupa Kode Akses</div>
            <div className="auth-subtitle">
              Masukkan email dan No HP yang Anda gunakan saat mengajukan kerja sama. Kode akses akan dikirim ulang ke email Anda.
            </div>

            <form onSubmit={handleSubmit} style={{ width: '100%', display: 'flex', flexDirection: 'column', alignItems: 'center', gap: 14, marginTop: 4 }} noValidate>
