...
import {
  ...
  Handshake, GraduationCap, Loader2, UserCheck
} from 'lucide-react';
        ...
        <div style={{ ...shellStyle, maxWidth: 560 }} className="reveal">
          <div style={coreStyle}>
            <div style={{ textAlign:'center', marginBottom:28 }}>
              ...
              <div style={{ fontSize:26, fontWeight:800, color:'#0a2e24', letterSpacing:'-0.03em' }}>Pengajuan Berhasil</div>
              <div style={{ fontSize:13.5, color:'#5b6b66', marginTop:8, display:'flex', alignItems:'center', justifyContent:'center', gap:7 }}>
            ...
            <div style={kodeShell}>
              <div style={kodeCore}>
                ...
                <div style={{ fontSize:36, fontWeight:800, letterSpacing:6, color:'#0F6E56', margin:'10px 0 16px', fontFamily:'monospace' }}>
                  {hasil.kodeTracking}
                </div>
                ...
                <div style={{ fontSize:11.5, color:'#5b6b66', marginTop:14, display:'flex', alignItems:'center', justifyContent:'center', gap:5 }}>
            ...
            {hasil.adaDokumenMitra && (
              <div style={infoCard('#1E40AF', 'rgba(37,99,235,0.07)', 'rgba(37,99,235,0.15)')}>
            ...
            <div style={infoCard('#78350F', 'rgba(217,119,6,0.07)', 'rgba(217,119,6,0.16)')}>
            ...
            <div style={{ margin:'22px 0' }}>
              <div style={{ fontSize:13.5, fontWeight:700, marginBottom:16, display:'flex', alignItems:'center', gap:8, color:'#0a2e24' }}>
                ...
                <div key={s} style={{ display:'flex', alignItems:'center', gap:14 }}>
                  <div style={{ display:'flex', flexDirection:'column', alignItems:'center' }}>
                    <div style={{ width:30, height:30, borderRadius:'50%', display:'flex', alignItems:'center', justifyContent:'center',
                      background: i===0 ? 'linear-gradient(145deg,#14a37f,#0F6E56)' : '#eef1f0',
                      color: i===0 ? '#fff' : '#9aa5a1', fontSize:12, fontWeight:700, flexShrink:0,
                      boxShadow: i===0 ? '0 4px 12px -2px rgba(15,110,86,0.4)' : 'none' }}>
                      {i === 0 ? <Check size={15} strokeWidth={2.2} /> : i + 1}
                    </div>
                    {i < STATUS_ALUR.length - 1 && (
                      <div style={{ width:2, height:26, background: i===0 ? 'linear-gradient(#0F6E56,#cfe9e0)' : '#eef1f0', marginTop:3 }} />
                    )}
                  </div>
                  <div style={{ fontSize:13, color: i===0 ? '#0a2e24' : '#9aa5a1', fontWeight: i===0 ? 700 : 500, display:'flex', alignItems:'center', gap:8, flexWrap:'wrap', paddingBottom: i<STATUS_ALUR.length-1 ? 14 : 0 }}>
            ...
            <div style={{ display:'flex', gap:10 }}>
              <a href="/cek-pengajuan" style={{ ...ctaPill, flex:1, justifyContent:'center', textDecoration:'none' }} className="cta">
                Cek Status
      ...
      <div style={shellStyle} className={mounted ? 'reveal' : ''}>
        <div style={coreStyle}>
          ...
          <div style={{ textAlign:'center', marginBottom:26 }}>
            ...
            <h1 style={{ fontSize:27, fontWeight:800, color:'#0a2e24', letterSpacing:'-0.035em', margin:'14px 0 6px', lineHeight:1.1 }}>
              Form Pengajuan<br/>Kerja Sama
            </h1>
            <p style={{ fontSize:13.5, color:'#5b6b66', margin:0 }}>Ajukan kerja sama kelembagaan dengan tim Pokja Humker</p>
          </div>

          <div style={{ marginBottom:24 }}>
            <div style={{ display:'flex', justifyContent:'space-between', fontSize:11.5, color:'#5b6b66', marginBottom:7, fontWeight:600 }}>
              <span>Progress Pengisian</span><span style={{ color:'#0F6E56' }}>{formProgress}%</span>
            </div>
            <div style={{ height:7, background:'#eef1f0', borderRadius:100, overflow:'hidden', boxShadow:'inset 0 1px 2px rgba(10,46,36,0.06)' }}>
              <div style={{ height:'100%', borderRadius:100, width:`${formProgress}%`, background:'linear-gradient(90deg,#0F6E56,#19b894)', transition:`width 0.7s ${EASE}` }} />
            </div>
          </div>
          ...
          <form onSubmit={handleSubmit}>
                  ...
                  return (
                    <button key={j} type="button" onClick={()=>{ setJenis(j); setJurusan(''); }}
                      style={{ padding:'18px 12px', borderRadius:18, cursor:'pointer', fontFamily:FONT, textAlign:'center',
                        border:`1.5px solid ${on ? '#0F6E56' : 'rgba(10,46,36,0.10)'}`,
                        background: on ? 'linear-gradient(160deg,#f1fbf7,#ddf3ea)' : '#fff',
                        color: on ? '#0a3e30' : '#54635e', transition:`all 0.45s ${EASE}`,
                        boxShadow: on ? '0 8px 22px -10px rgba(15,110,86,0.45)' : '0 1px 2px rgba(10,46,36,0.04)',
            ...
            <div style={{ ...nestGroup, ...fieldDelay(4) }} className="fld">
              ...
              <div onDragEnter={handleDragEnter} onDragLeave={handleDragLeave} onDragOver={handleDragOver} onDrop={handleDrop}
                ...
                style={{ border:`1.5px dashed ${isDragging ? '#0F6E56' : 'rgba(10,46,36,0.18)'}`, borderRadius:16, padding:'1.6rem', textAlign:'center', cursor:'pointer', transition:`all 0.4s ${EASE}`, background: isDragging ? 'rgba(15,110,86,0.05)' : '#fff' }}>
                {file ? (
                  <div style={{ display:'flex', alignItems:'center', justifyContent:'center', gap:12, flexWrap:'wrap' }}>
                    <FileText size={30} strokeWidth={1.4} style={{ color:'#0F6E56' }} />
                    <div style={{ textAlign:'left' }}>
                      <div style={{ fontSize:13.5, fontWeight:600, color:'#0a2e24' }}>{file.name}</div>
                      <div style={{ fontSize:11.5, color:'#5b6b66' }}>{(file.size/1024).toFixed(1)} KB</div>
                    </div>
                  ...
                  <div>
                    <div style={{ width:46, height:46, borderRadius:'50%', background:'#f1fbf7', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 12px' }}>
                      <Upload size={20} strokeWidth={1.6} style={{ color:'#0F6E56' }} />
                    </div>
                    <div style={{ fontSize:13.5, color:'#3a4742', fontWeight:500 }}>{isDragging ? 'Lepaskan di sini' : 'Tarik & letakkan, atau klik untuk pilih'}</div>
                    <div style={{ fontSize:11.5, color:'#9aa5a1', marginTop:5 }}>.doc / .docx</div>
                  </div>
                )}
              </div>
            ...
            <div style={{ ...nestGroup, ...fieldDelay(6) }} className="fld">
              ...
              <div>
                <label style={subLabel}><Phone size={12} strokeWidth={1.7} /> No. WhatsApp</label>
                <input style={inp(focusedField==='wa')} type="tel" value={noWa} onChange={e=>setNoWa(e.target.value)}
            ...
            <button type="submit" disabled={loading || !!fileError}
              style={{ ...ctaPill, width:'100%', justifyContent:'center', height:54, fontSize:15, marginTop:8,
                opacity:(loading || !!fileError) ? 0.6 : 1, cursor:(loading || !!fileError) ? 'not-allowed' : 'pointer' }} className="cta">
            ...
            <div style={{ marginTop:18, fontSize:12.5, color:'#9aa5a1', textAlign:'center' }}>
              Sudah punya kode? <a href="/cek-pengajuan" style={{ color:'#0F6E56', textDecoration:'none', fontWeight:700, display:'inline-flex', alignItems:'center', gap:4 }}>Cek status <ArrowRight size={13} strokeWidth={1.9} /></a>
            </div>
          </form>
        </div>
      </div>
...
function GlobalStyle() {
  return (
    <style>{`
      ...
      .cta:hover { transform: translateY(-2px); box-shadow: 0 14px 30px -10px rgba(15,110,86,0.5) !important; }
...
const pageStyle: React.CSSProperties = {
  minHeight: '100dvh',
  background: 'radial-gradient(1200px 600px at 50% -10%, #e6f4ee 0%, rgba(230,244,238,0) 60%), linear-gradient(180deg,#f7f9f8,#eef2f0)',
  padding: '3rem 1.25rem', display:'flex', alignItems:'flex-start', justifyContent:'center',
};
const shellStyle: React.CSSProperties = {
  maxWidth: 600, width:'100%', background:'rgba(255,255,255,0.5)',
  border:'1px solid rgba(10,46,36,0.06)', borderRadius:34, padding:8,
  boxShadow:'0 1px 2px rgba(10,46,36,0.04), 0 40px 80px -40px rgba(10,46,36,0.22)',
};
const coreStyle: React.CSSProperties = {
  background:'#fff', borderRadius:27, padding:'2.4rem 2.1rem',
  boxShadow:'inset 0 1px 1px rgba(255,255,255,0.9)',
};
const badgeIcon: React.CSSProperties = { width:74, height:74, borderRadius:24, background:'linear-gradient(150deg,#16b08a,#0F6E56)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto 16px', boxShadow:'0 16px 34px -12px rgba(15,110,86,0.55)' };
const badgeIconSm: React.CSSProperties = { width:58, height:58, borderRadius:20, background:'linear-gradient(150deg,#16b08a,#0F6E56)', display:'flex', alignItems:'center', justifyContent:'center', margin:'0 auto', boxShadow:'0 14px 30px -12px rgba(15,110,86,0.5)' };
const eyebrow: React.CSSProperties = { fontSize:10, color:'#0F6E56', textTransform:'uppercase', letterSpacing:'0.2em', fontWeight:700, display:'flex', alignItems:'center', justifyContent:'center', gap:6 };
const eyebrowCenter: React.CSSProperties = { display:'inline-block', fontSize:9.5, color:'#0F6E56', textTransform:'uppercase', letterSpacing:'0.22em', fontWeight:700, background:'#eef9f4', padding:'6px 14px', borderRadius:100, marginBottom:18 };
const kodeShell: React.CSSProperties = { background:'linear-gradient(160deg,#f1fbf7,#e2f3eb)', border:'1px solid rgba(15,110,86,0.14)', borderRadius:24, padding:7, marginBottom:16 };
...
const pillNow: React.CSSProperties = { fontSize:9.5, background:'#dcf3ea', color:'#0a5c47', padding:'3px 10px', borderRadius:100, fontWeight:700, textTransform:'uppercase', letterSpacing:'0.08em' };
const fieldGroup: React.CSSProperties = { marginBottom:18 };
const nestGroup: React.CSSProperties = { marginBottom:18, background:'linear-gradient(170deg,#fafcfb,#f3f7f5)', padding:'1.25rem', borderRadius:20, border:'1px solid rgba(10,46,36,0.06)' };
...
const req: React.CSSProperties = { fontStyle:'normal', fontSize:9, color:'#0F6E56', background:'#eef9f4', padding:'2px 7px', borderRadius:100, marginLeft:'auto', textTransform:'uppercase', letterSpacing:'0.08em', fontWeight:700 };
const opt: React.CSSProperties = { fontSize:9.5, color:'#9aa5a1', fontWeight:500, textTransform:'none', letterSpacing:0, marginLeft:'auto' };
const hint: React.CSSProperties = { fontSize:11, color:'#9aa5a1', marginTop:7, lineHeight:1.6, display:'flex', alignItems:'center', gap:6 };
const inp = (focus: boolean): React.CSSProperties => ({
  width:'100%', padding:'13px 15px', borderRadius:13, fontSize:13.5, fontFamily:"'Plus Jakarta Sans', sans-serif", boxSizing:'border-box', outline:'none',
  background: focus ? '#fff' : '#fafcfb',
  border:`1.5px solid ${focus ? '#0F6E56' : 'rgba(10,46,36,0.10)'}`,
  boxShadow: focus ? '0 0 0 4px rgba(15,110,86,0.08)' : 'none',
  transition:'all 0.4s cubic-bezier(0.32,0.72,0,1)', color:'#0a2e24',
});
const ctaPill: React.CSSProperties = { display:'inline-flex', alignItems:'center', gap:10, padding:'13px 16px 13px 24px', borderRadius:100, border:'none', background:'linear-gradient(135deg,#13987a,#0F6E56)', color:'#fff', fontSize:14, fontWeight:700, cursor:'pointer', fontFamily:"'Plus Jakarta Sans', sans-serif", boxShadow:'0 10px 24px -10px rgba(15,110,86,0.5)' };
...
const ghostPill: React.CSSProperties = { display:'inline-flex', alignItems:'center', justifyContent:'center', gap:7, padding:'13px 22px', borderRadius:100, border:'1.5px solid rgba(10,46,36,0.12)', background:'#fff', color:'#3a4742', fontSize:13.5, fontWeight:700, cursor:'pointer', fontFamily:"'Plus Jakarta Sans', sans-serif" };
