...
// Maksimal 5 percobaan kode akses per 15 menit, per alamat IP.
const LIMIT = 5;
const WINDOW_MS = 15 * 60 * 1000;

export async function POST(req: NextRequest) {
  const ip = getClientIp(req);
  const limitResult = checkRateLimit(`login-mitra:${ip}`, LIMIT, WINDOW_MS);

  if (!limitResult.allowed) {
    return NextResponse.json(
      { message: `Terlalu banyak percobaan. Coba lagi dalam ${Math.ceil(limitResult.retryAfterSeconds / 60)} menit.` },
  ...
  try {
    ...
    const idx  = rows.findIndex(
      r => String(r[COL.KODE]).trim().toUpperCase() === kode.trim().toUpperCase()
    );
    if (idx === -1) {
