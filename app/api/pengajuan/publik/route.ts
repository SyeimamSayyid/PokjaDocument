...
// Kolom "Pengajuan Mitra" (0-based) — VERSI BARU
// 0  ID Pengajuan
// 1  ID Mitra
// 2  Nama Institusi
// 3  Jenis
...
// 11 Tanggal Submit
// 12 Catatan Admin
// 13 Jurusan/Prodi       ← khusus PKS (kosong untuk MOU)
// 14 File Dokumen ID
// 15 File Dokumen URL
// 16 Nama File Dokumen
// 17 Divisi              ← tidak diisi dari form publik (placeholder)
// 18 Nama PIC            ← penanggung jawab (WAJIB)
...
export async function POST(req: NextRequest) {
  try {
    ...
    // ── #6: Jurusan hanya untuk PKS ─────────────────────────
    const jurusanFinal = jenis === 'PKS' ? (jurusan?.trim() || '') : '';
    ...
    // Simpan ke sheet "Pengajuan Mitra" (19 kolom, index 0–18)
    await appendRow('Pengajuan Mitra', [
      idPengajuan,             // 0
      idMitra || '',           // 1
      namaInstitusi.trim(),    // 2
      jenis,                   // 3
      deskripsi.trim(),        // 4
      tanggalKegiatan || '',   // 5
      biaya || '',             // 6
      ...
      'Diajukan',              // 9
      kodeTracking,            // 10
      formatTanggalWaktu(now), // 11
      '',                      // 12 catatan admin
      jurusanFinal,            // 13 jurusan/prodi (PKS saja)
      fileDokumenId,           // 14
      fileDokumenUrl,          // 15
      fileDokumenNama,         // 16
      '',                      // 17 divisi (tidak dari form publik)
