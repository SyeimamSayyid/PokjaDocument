...
interface Arsip {
  ...
  diarsipkanOleh: string; tglDiarsipkan: string;
}
    ...
    try {
      ...
      const r = await fetch('/api/arsip-dokumen', {
        ...
        body: JSON.stringify({
          ...
          tglBerlaku: fBerlaku, tglBerakhir: fBerakhir,
  ...
  return (
    <div style={{ minHeight:'100vh', background:'linear-gradient(180deg,#f7f9f8,#eef2f0)', fontFamily: FONT }}>
      ...
      <nav style={navStyle}>
        ...
        <div style={{ fontWeight:700, fontSize:13.5, flex:1, textAlign:'center', color:'#0a2e24' }}>Arsip Dokumen</div>
      ...
      <div style={{ maxWidth:1000, margin:'0 auto', padding:'1.5rem 1.25rem 3rem' }}>
        ...
        {msg   && <div style={{ ...msgBox('#085041','#E1F5EE'), marginBottom:14 }} className="fld">{msg}</div>}
        {error && <div style={{ ...msgBox('#A32D2D','#FCEBEB'), marginBottom:14 }} className="fld">{error}</div>}

        <div style={{ ...eyebrow, marginBottom:6 }}>Internal · Hanya Admin</div>
        <p style={{ fontSize:12, color:'#5b6b66', marginTop:0, marginBottom:18, lineHeight:1.6 }}>
          Wadah untuk kerja sama lama sebelum sistem ini ada. Diarsipkan manual oleh admin, tidak tampil di halaman publik maupun sisi mitra.
        </p>
            ...
            <div style={coreStyle}>
              ...
              <form onSubmit={submitArsip}>
                ...
                <div style={{ marginBottom:16 }}>
                  <label style={labelSt}>Berkas Dokumen * <span style={{ fontWeight:400, color:'#9aa5a1' }}>(Word / PDF / Scan JPG-PNG, maks 5MB)</span></label>
                  ...
                  {fFile && <div style={{ fontSize:11, color:'#0F6E56', marginTop:6 }}>✓ {fFile.name} ({(fFile.size/1024).toFixed(0)} KB)</div>}
                  ...
                  <div style={{ display:'flex', justifyContent:'space-between', alignItems:'flex-start', gap:10, flexWrap:'wrap' }}>
                    <div style={{ flex:1, minWidth:200 }}>
                      <div style={{ display:'flex', gap:8, alignItems:'center', marginBottom:6, flexWrap:'wrap' }}>
                        <span style={{ ...pill, background:a.jenis==='MOU'?'#E6F1FB':'#FAEEDA', color:a.jenis==='MOU'?'#0C447C':'#854F0B' }}>{a.jenis}</span>
                        <span style={{ fontSize:10, color:'#9aa5a1' }}>Diarsipkan {a.tglDiarsipkan}{a.diarsipkanOleh ? ` oleh ${a.diarsipkanOleh}` : ''}</span>
                      </div>
                      <div style={{ fontSize:14.5, fontWeight:700, color:'#0a2e24' }}>{a.judul}</div>
                      <div style={{ fontSize:12.5, color:'#0F6E56', fontWeight:600, marginTop:2 }}>🏢 {a.namaInstitusi}</div>
                      <div style={{ fontSize:11, color:'#5b6b66', marginTop:6 }}>Berlaku: {a.tglBerlaku} s.d. {a.tglBerakhir}</div>
                      <div style={{ fontSize:11, color:'#5b6b66', marginTop:2 }}>
                        PIC: {a.namaPIC}{a.emailPIC ? ` · ${a.emailPIC}` : ''}{a.waPIC ? ` · ${a.waPIC}` : ''}
                      </div>
                      {a.catatan && <div style={{ fontSize:11, color:'#9aa5a1', marginTop:6, fontStyle:'italic' }}>{a.catatan}</div>}
                    </div>
                    {a.fileUrl && (
                      <a href={a.fileUrl} target="_blank" rel="noopener noreferrer" style={{ ...btnSm, textDecoration:'none', flexShrink:0 }} className="btn-hover">
                        📄 Lihat Berkas
                      </a>
                    )}
                  </div>
...
const shellStyle: React.CSSProperties = { background:'rgba(255,255,255,0.6)', border:'1px solid rgba(10,46,36,0.06)', borderRadius:20, padding:6, boxShadow:'0 1px 2px rgba(10,46,36,0.03), 0 20px 40px -30px rgba(10,46,36,0.18)' };
...
const cardTitle: React.CSSProperties = { fontSize:13, fontWeight:700, marginBottom:14, color:'#0a2e24' };
const labelSt: React.CSSProperties = { display:'block', fontSize:11, color:'#3a4742', marginBottom:5, fontWeight:700 };
const subLabel: React.CSSProperties = { display:'block', fontSize:10.5, color:'#5b6b66', marginBottom:4, fontWeight:600 };
const hintText: React.CSSProperties = { fontSize:10, color:'#9aa5a1', marginTop:6 };
...
const nestGroup: React.CSSProperties = { background:'#fafcfb', border:'1px solid rgba(10,46,36,0.06)', borderRadius:14, padding:'0.9rem 1rem' };
const btnPrimary: React.CSSProperties = { padding:'9px 16px', borderRadius:11, border:'none', background:'linear-gradient(135deg,#7c3aed,#5B21B6)', color:'#fff', fontSize:12.5, fontWeight:700, cursor:'pointer', fontFamily:FONT, boxShadow:'0 6px 16px -6px rgba(91,33,182,0.45)' };
const btnSm: React.CSSProperties = { padding:'7px 13px', borderRadius:10, border:'1.5px solid rgba(10,46,36,0.10)', background:'#fff', color:'#3a4742', fontSize:11, fontWeight:600, cursor:'pointer', fontFamily:FONT, whiteSpace:'nowrap' };
const pill: React.CSSProperties = { fontSize:10, fontWeight:700, padding:'3px 10px', borderRadius:100 };
const eyebrow: React.CSSProperties = { display:'inline-block', fontSize:9.5, color:'#5B21B6', textTransform:'uppercase', letterSpacing:'0.14em', fontWeight:700, background:'#EDE9FE', padding:'4px 11px', borderRadius:100 };
const emptyBox: React.CSSProperties = { padding:'2.5rem', textAlign:'center', color:'#9aa5a1', fontSize:12, background:'#f9fafb', borderRadius:16 };
...
const jenisBtn: React.CSSProperties = { flex:1, padding:'9px', borderRadius:10, border:'1.5px solid rgba(10,46,36,0.10)', background:'#fff', color:'#5b6b66', fontSize:12, fontWeight:700, cursor:'pointer', fontFamily:FONT };
const jenisBtnActive: React.CSSProperties = { border:'1.5px solid #0F6E56', background:'linear-gradient(160deg,#f1fbf7,#e2f3eb)', color:'#0a3e30' };
const filterPill: React.CSSProperties = { padding:'8px 15px', borderRadius:100, borderWidth:1.5, borderStyle:'solid', borderColor:'rgba(10,46,36,0.10)', background:'#fff', color:'#5b6b66', fontSize:11.5, fontWeight:600, cursor:'pointer', fontFamily:FONT };
const filterPillActive: React.CSSProperties = { background:'#5B21B6', borderColor:'#5B21B6', color:'#fff' };